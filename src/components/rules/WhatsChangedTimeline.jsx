    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-sm space-y-2">
        <span className="text-xs font-extrabold uppercase tracking-wider text-purple-700 bg-purple-50 px-3 py-1 rounded-full border border-purple-200">
          {isBengali ? 'আইনগত পরিবর্তন ও ইতিহাস' : 'Statutory Legislative History'}
        </span>
        <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">
          {isBengali ? 'বাংলাদেশ আয়করে কী কী পরিবর্তন এসেছে?' : "What's changed in Bangladesh Income Tax?"}
        </h2>
      ...
      <div className="space-y-4 sm:space-y-6">
        {WHATS_CHANGED_TIMELINE.map((item) => {
          ...
          const badgeText = isBengali && item.badgeBn ? item.badgeBn : item.badge;
          const topicText = isBengali && item.topicBn ? item.topicBn : item.topic;
          const titleText = isBengali && item.titleBn ? item.titleBn : item.title;
          const oldRuleText = isBengali && item.oldRuleBn ? item.oldRuleBn : item.oldRule;
          const newRuleText = isBengali && item.newRuleBn ? item.newRuleBn : item.newRule;
          const impactText = isBengali && item.impactBn ? item.impactBn : item.impact;
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-500">
                      {isBengali ? 'বিষয়:' : 'Topic:'} {topicText}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-1.5 px-3 py-1 bg-slate-100 rounded-xl text-xs font-bold text-slate-700 self-start sm:self-auto shrink-0">
                  ...
                  <span>{isBengali ? (item.effectiveAYBn || `কার্যকর: করবর্ষ ${item.effectiveAY}`) : `Effective: AY ${item.effectiveAY}`}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-4 sm:p-5 rounded-2xl bg-red-50/50 border border-red-200/70 space-y-1.5">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-red-700 block">
                    {isBengali ? 'পূর্ববর্তী নিয়ম (পুরাতন)' : 'Previous Statutory Rule (Old)'}
                  </span>
                ...
                <div className="p-4 sm:p-5 rounded-2xl bg-emerald-50/60 border border-emerald-200/80 space-y-1.5">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-emerald-800 block">
                    {isBengali ? 'হালনাগাদ কার্যকরী নিয়ম (নতুন)' : 'Enacted Official Rule (New)'}
                  </span>
              ...
              <div className="pt-3 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                <div className="text-slate-600">
                  <strong className="text-slate-800">{isBengali ? 'আর্থিক প্রভাব:' : 'Financial Impact:'}</strong> {impactText}
                </div>

                <div className="flex items-center gap-2 font-semibold text-slate-500 shrink-0">
                  <FileText className="w-3.5 h-3.5 text-slate-400" />
                  <span className="truncate max-w-xs">{isBengali ? (item.officialSourceBn || item.officialSource) : item.officialSource}</span>
                </div>
              </div>
            </div>
          );
