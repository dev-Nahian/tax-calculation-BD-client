...
export const CalculatorPreview = () => {
  ...
  const chartData = [
    { name: '0% Exemption', amount: Math.min(taxable, threshold), tax: 0, fill: '#10B981' },
    { name: '5% Slab', amount: Math.min(Math.max(0, remaining), 100000), tax: Math.min(Math.max(0, remaining), 100000) * 0.05, fill: '#3B82F6' },
    { name: '10% Slab', amount: Math.min(Math.max(0, remaining - 100000), 400000), tax: Math.min(Math.max(0, remaining - 100000), 400000) * 0.1, fill: '#F59E0B' },
    { name: '15%+ Slabs', amount: Math.max(0, remaining - 500000), tax: Math.max(0, remaining - 500000) * 0.15, fill: '#EF4444' },
  ];

  const totalEstimatedTax = chartData.reduce((acc, curr) => acc + curr.tax, 0);
  const finalPayable = Math.max(totalEstimatedTax > 0 ? totalEstimatedTax : 0, taxable > threshold ? 5000 : 0);
            ...
            <div>
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-2">
                  <label htmlFor="salarySlider" className="text-xs font-bold uppercase tracking-wider text-slate-700">
                    Gross Annual Income
                  </label>
              ...
              <div className="grid grid-cols-2 gap-3 pt-4 border-t border-slate-200">
                <div className="bg-white p-3.5 rounded-xl border border-slate-200">
                  <span className="text-xs text-slate-500 block">Est. Net Tax</span>
          ...
          <div className="lg:col-span-6 bg-slate-900 text-white rounded-2xl p-6 sm:p-8 flex flex-col justify-between shadow-xl">
                        ...
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="bg-slate-800 border border-slate-700 p-2.5 rounded-lg text-xs shadow-lg">
                              ...
                              <p className="text-slate-300">Amount in slab: {formatBDT(data.amount)}</p>
                              <p className="text-emerald-400 font-semibold">Tax in slab: {formatBDT(data.tax)}</p>
                            </div>
                          );
                        }
                        return null;
            ...
            <div className="pt-4 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
              <span className="flex items-center gap-2">
                ...
                First ৳3.5L is 100% Tax Free
              </span>
              <span className="text-slate-300">AY 2024-2025</span>
            </div>
          </div>
        </div>
      </div>
